...
**Recommended and alternative.** The page does not ask "which Go model is best" but "what should I use". Every measured model paid per token at its cheapest usable provider, and every Go model whose allowance lasts the month at the fee, are options; the same rule as the main page picks among them, with the fee as the budget: the highest Coding Index costing at most the fee, options within `CLOSE_POINTS` (3) of it count as equal and the cheapest wins. So a model that scores higher and costs less per token is recommended over Go. The alternative is the best option of the other kind; on the Go side it is only a model for which Go costs less than paying for that same model per token, never the recommended model again at a higher price.
