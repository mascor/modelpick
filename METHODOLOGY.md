...
- **Data retention by providers.** OpenRouter does not publish, per provider, whether prompts are retained, so the configuration we publish cannot filter on it without risking that the pinned provider refuses every request. OpenRouter lets you exclude such providers once for your whole account, in its privacy settings.
